...
CREATE TABLE IF NOT EXISTS client(clientID BIGINT, clientName VARCHAR (128), clientType VARCHAR (128), orderID BIGINT, FOREIGN KEY(orderID) references order(orderID) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS car (carID BIGINT, brand VARCHAR (128), model VARCHAR (128), carYear INTEGER, engine VARCHAR (128), clientID BIGINT FOREIGN KEY(clientID) references client(clientID) ON DELETE CASCADE);


CREATE TABLE IF NOT EXISTS employee (employeeID BIGINT, employeeName VARCHAR (128), orderID BIGINT, FOREIGN KEY(orderID) references order(orderID) ON DELETE CASCADE);
